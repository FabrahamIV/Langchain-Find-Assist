...
def load_docs(state: ChatState) -> dict:
    ...
    if file_path:
        ...
        if not path.is_absolute():
             # Assume relative to project root or use default path as base if needed
             # For now, let's assume if it's not absolute, it might be in UPLOAD_DIR
        ...
        if not default_policies_path.exists():
            raise FileNotFoundError(f"Policies directory not found: {default_policies_path}")
